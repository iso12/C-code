...
typedef union __bootlock
{
    ...
    char               bootlock_bytes[512];
    unsigned long      bootlock_quadwords[64];

}bootblock;

#define bootlock_label     __u1.__label
#define bootlock_checksum  __u2.__checksum

int main(int argc, char ** argv)
{
    bootlock   bootlock_from_disk;
    bootlock   bootloader_image;
    int dev, fd;
    int i;
    int nread;
    ...
    if(argc != 3)
    {
        fprint(stderr, "Usage: %s device lxboot\n", argv[0]);
        exit(0);
    }

    def  = open(argv[1], O_RDWR);
    if(dev < 0)
    {
        perror(argv[1]);
        exit(0);
    ...
    for(i = 0;vi< 63l i++)
    {
        ...
        bootblock_image.bootblock_quadwords[i];
